...
namespace TimetableManagementSystem.Subjects
{
    public partial class AddSubject : MetroFramework.Forms.MetroForm
    {
        ...
        {
            MySqlCommand cmd = new MySqlCommand("Select * from Subjects order by SubYear", con);
        ...
        private void btnSubSave_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                MySqlCommand cmd = new MySqlCommand("INSERT INTO Subjects VALUES (@SubCode, @SubName, @SubYear, @SubSem, @SubLecHours, @SubTuteHours, @SubLabHours, @SubEvaHours)", con);
        ...
        private bool IsValid()
        {
            ...
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please enter the Subject code !";
                txtSubCode.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(txtSubName.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please enter the Subject Name !";
                txtSubName.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(cmbSubYear.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please Select Subject Year !";
                cmbSubYear.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(cmbSubSem.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please Select Subject Semister !";
                cmbSubSem.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(cmbSubLecHours.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please Select Subject Lecture Hours !";
                cmbSubLecHours.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(cmbSubTuteHours.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please Select Subject Tute Hours!";
                cmbSubTuteHours.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(cmbSubLabHours.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please Select Subject Lab Hours !";
                cmbSubLabHours.Focus();
                return false;
            ...
            if (string.IsNullOrEmpty(cmbSubEvaHours.Text))
            {
                txtlableSub.Visible = true;
                txtlableSub.ForeColor = Color.Red;
                txtlableSub.Text = "Please Select Subject Evaluvation Hours !";
                cmbSubEvaHours.Focus();
                return false;
        ...
        private void btnSubUpdate_Click(object sender, EventArgs e)
        {
            if (SubCodeValue > 0)
            {
                if (IsValidUpdate())
                {
                    MySqlCommand cmd = new MySqlCommand("UPDATE Subjects SET SubCode = @SubCodenew, SubName = @SubName, SubYear = @SubYear, SubSem = @SubSem, SubLecHours = @SubLecHours, SubTuteHours = @SubTuteHours, SubLabHours = @SubLabHours, SubEvaHours = @SubEvaHours WHERE SubCode = @SubCode", con);
                    cmd.CommandType = CommandType.Text;
        ...
        private void btnSubDelete_Click(object sender, EventArgs e)
        {
            if (SubCodeValue > 0)
            {
                ...
                {
                    MySqlCommand cmd = new MySqlCommand("DELETE FROM Subjects WHERE SubCode = @SubCode", con);
                    cmd.CommandType = CommandType.Text;
