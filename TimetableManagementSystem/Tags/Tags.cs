    ...
    public partial class Tags : MetroFramework.Forms.MetroForm
    {
        ...
        private void tagNameSearchBox_TextChanged(object sender, EventArgs e)
        {
            if (tagNameSrtDrpDwn.Text == "")
            {
                MySqlConnection con = Config.con;
                con.Open();
                MySqlDataAdapter sda = new MySqlDataAdapter("SELECT * FROM Tags WHERE TagName LIKE '%" + tagNameSearchBox.Text + "%'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                tagNameData.DataSource = dt;
                con.Close();
            ...
            else if (tagNameSrtDrpDwn.Text == "ID")
            {
                MySqlConnection con = Config.con;
                con.Open();
                MySqlDataAdapter sda = new MySqlDataAdapter("SELECT * FROM Tags WHERE id LIKE '%" + tagNameSearchBox.Text + "%'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                tagNameData.DataSource = dt;
                con.Close();
