    ...
    public partial class Add_Number_of_Working_Days : MetroFramework.Forms.MetroForm
    {
        ...
        public Add_Number_of_Working_Days(int type)
        {
            ...
            if (type == 0)
            {
                ...
                con.Open();
                MySqlCommand LecCommand = new MySqlCommand(initialQuery, con);

                using (MySqlDataReader reader = LecCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
        ...
        MySqlConnection con = Config.con;
        public int EntryID;
        ...
        {
            MySqlCommand check_User_Name = new MySqlCommand("SELECT COUNT(*) FROM WorkingDays", con);
            con.Open();

           // int UserExist = (int)check_User_Name.ExecuteScalar();
            object anObj = check_User_Name.ExecuteScalar();

            int UserExist = (anObj == null ? -1 : Convert.ToInt32(anObj.ToString()));
        ...
        {
            MySqlCommand cmd = new MySqlCommand("select * from WorkingDaysWD ", con);
            DataTable wddt = new DataTable();

            con.Open();

            MySqlDataReader wddr = cmd.ExecuteReader();
            wddt.Load(wddr);
            con.Close();
        ...
        {
            MySqlCommand cmd = new MySqlCommand("select EntryID, Day , Timeslot from TimeSlot ", con);
            DataTable ts = new DataTable();

            con.Open();

            MySqlDataReader tss = cmd.ExecuteReader();
            ts.Load(tss);
            con.Close();
        ...
        {
            MySqlCommand cmd = new MySqlCommand("select * from WorkingTime", con);
            DataTable wdt = new DataTable();

            con.Open();

            MySqlDataReader wdr = cmd.ExecuteReader();
            wdt.Load(wdr);
            con.Close();
        ...
        {
            MySqlCommand cmd = new MySqlCommand("select * from WorkingDays", con);
        ...
        private void btnAddNWD_Click(object sender, EventArgs e)
        {
            ...
            if (isValid())
            {
                ...
                {
                    avalibility(EntryID);
                    MySqlCommand cmd = new MySqlCommand("INSERT into WorkingDays values(@NumberOfWorkingDays)", con);
                ...
                {
                    string query = "SELECT NumberOfWorkingDays FROM WorkingDays";
                    con.Open();
                    ...
                    {
                        avalibility(EntryID);
                        MySqlCommand cmd = new MySqlCommand("UPDATE WorkingDays SET NumberOfWorkingDays = @NumberOfWorkingDays", con);
                        cmd.CommandType = CommandType.Text;
        ...
        private void btneditNWD_Click(object sender, EventArgs e)
        {
            if (EntryID > 0)
            {
                ...
                if (temp <= 7)
                {
                    MySqlCommand cmd = new MySqlCommand("UPDATE WorkingDays SET  [Number Of Working Days] = @NumberOfWorkingDays WHERE EntryID= @EntryID", con);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@NumberOfWorkingDays", numericUpDownNWD.Text);
        ...
        private void btndeleteNWD_Click(object sender, EventArgs e)
        {
            if (EntryID > 0)
            {
                MySqlCommand cmd = new MySqlCommand("DELETE FROM WorkingDays WHERE EntryID = @EntryID", con);
                cmd.CommandType = CommandType.Text;
        ...
        private void btnaddWT_Click(object sender, EventArgs e)
        {
            if (iswTimeValid())
            {
                MySqlCommand cmd = new MySqlCommand("INSERT INTO WorkingTime VALUES('',@Hours, @Minutes)", con);
                cmd.CommandType = CommandType.Text;
        ...
        private void btneditWT_Click(object sender, EventArgs e)
        {
            if (WorkingTimeID > 0)
            {
                ...
                MySqlCommand cmd = new MySqlCommand("UPDATE WorkingTime SET  Hours = @Hours , Minutes = @Minutes WHERE WorkingTimeID= @WorkingTimeID", con);
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@Hours", numericUpDownHours.Text);
                cmd.Parameters.AddWithValue("@Minutes", numericUpDownMinutes.Text);
        ...
        private void btndeleteWT_Click(object sender, EventArgs e)
        {
            if (WorkingTimeID > 0)
            {
                MySqlCommand cmd = new MySqlCommand("DELETE FROM WorkingTime WHERE WorkingTimeID = @WorkingTimeID", con);
                cmd.CommandType = CommandType.Text;
        ...
        private void btnaddWD_Click_1(object sender, EventArgs e)
        {
            ...
            {
                MySqlCommand cmd = new MySqlCommand("INSERT INTO WorkingDaysWD VALUES('',@Day1, @Day2, @Day3, @Day4, @Day5, @Day6, @Day7)", con);
                cmd.CommandType = CommandType.Text;
        ...
        private void btneditWD_Click(object sender, EventArgs e)
        {
            if (WorkingDaysWDID > 0)
            {
                ...
                MySqlCommand cmd = new MySqlCommand("UPDATE WorkingDaysWD SET  Day1 = @Day1, Day2 = @Day2, Day3 = @Day3, Day4 = @Day4, Day5 = @Day5, Day6 = @Day6 , Day7 = @Day7 WHERE WorkingDaysWDID= @WorkingDaysWDID", con);
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@Day1", cmbWd1.Text);
                cmd.Parameters.AddWithValue("@Day2", cmbWd2.Text);
                cmd.Parameters.AddWithValue("@Day3", cmbWd3.Text);
        ...
        private void btndeleteWD_Click(object sender, EventArgs e)
        {
            if (WorkingDaysWDID > 0)
            {
                MySqlCommand cmd = new MySqlCommand("DELETE FROM WorkingDaysWD  WHERE WorkingDaysWDID = @WorkingDaysWDID", con);
                cmd.CommandType = CommandType.Text;
        ...
        {
            MySqlCommand AddtoLecTime = new MySqlCommand("INSERT INTO Lecturer_Time VALUES(@LecID, @SessionID, @StartTime, @EndTime, @Dates)", con);
            ...
            MySqlCommand AddtoClassTime = new MySqlCommand("INSERT INTO Classroom_Time VALUES(@ClassID, @SessionID, @StartTime, @EndTime, @Dates)", con);
            ...
            MySqlCommand AddtoGroupTime = new MySqlCommand("INSERT INTO Group_Time VALUES(@GroupID, @SessionID, @StartTime, @EndTime, @Dates)", con);
        ...
        private void cmdsessionfaculty_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmdsessionfaculty.SelectedIndex == 0)
            {
                ...
                con.Open();
                MySqlCommand getClassRoomCommand = new MySqlCommand(getClassRoomQuery, con);

                using (MySqlDataReader reader = getClassRoomCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
            ...
            if (type == 2)
            {
                ...
                con.Open();
                MySqlCommand RoomCommand = new MySqlCommand(buildingQuery, con);

                using (MySqlDataReader reader = RoomCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
        ...
        private void metroComboBox1_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            if (type == 0)
            {
                ...
                con.Open();
                MySqlCommand LecCenterCommand = new MySqlCommand(CenterQuery, con);

                using (MySqlDataReader reader = LecCenterCommand.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        Lecturers1.Add(reader.GetString(0));
        ...
        private void cmdsessionlecturer_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            MySqlCommand LecIDCommand = new MySqlCommand(LecIDQuery, con);
            using (MySqlDataReader reader = LecIDCommand.ExecuteReader())
            {
                while (reader.Read())
                {
        ...
        private void cmdGTlecsession_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            con.Close();
            con.Open();
            MySqlCommand groupCommand = new MySqlCommand(groupQuery, con);

            using (MySqlDataReader reader = groupCommand.ExecuteReader())
            {
                while (reader.Read())
                {
            ...
            foreach (String days in day)
            {
                foreach (String hour in hours)
                {
                    ...
                    if (lectureFlag)
                    {
                       ...
                       " AND Day = '" + days + "'";
                        con.Open();
                        MySqlCommand sessCommand = new MySqlCommand(selectSessionQuery, con);

                        using (MySqlDataReader reader = sessCommand.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                if (reader.GetInt32(0) == 0)
                        ...
                        if (sessionFlag)
                        {
                           ...
                           " AND Day = '" + days + "'";
                            con.Open();
                            MySqlCommand selectGroupCommand = new MySqlCommand(selectSessionQuery, con);

                            using (MySqlDataReader reader = selectGroupCommand.ExecuteReader())
                            {
                                while (reader.Read())
                                {
                                    if (reader.GetInt32(0) == 0)
                            ...
                            if (groupFlag)
                            {
                               ...
                               " AND Day = '" + days + "'";
                                con.Open();
                                MySqlCommand subGroupCommand = new MySqlCommand(selectSubGroupQuery, con);

                                using (MySqlDataReader reader = subGroupCommand.ExecuteReader())
                                {
                                    while (reader.Read())
                                    {
                                        if (reader.GetInt32(0) == 0)
                                ...
                                if (subGroupFlag)
                                {
                                    ...
                                    con.Open();
                                    MySqlCommand checkLecCommand = new MySqlCommand(checkLecSQL, con);

                                    using (MySqlDataReader reader = checkLecCommand.ExecuteReader())
                                    {
                                        while (reader.Read())
                                        {
                                            if (reader.GetInt32(0) == 0)
                                    ...
                                    if (noLecFlag)
                                    {
                                        ...
                                        con.Open();
                                        MySqlCommand checkGroupCommand = new MySqlCommand(checkGroupSQL, con);

                                        using (MySqlDataReader reader = checkGroupCommand.ExecuteReader())
                                        {
                                            while (reader.Read())
                                            {
                                                if (reader.GetInt32(0) == 0)
        ...
        private void cmdsessionday_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            con.Open();
            MySqlCommand checkClassCommand = new MySqlCommand(checkClassSQL, con);

            using (MySqlDataReader reader = checkClassCommand.ExecuteReader())
            {
                while (reader.Read())
                {
        ...
        private void btnTSaddts_Click(object sender, EventArgs e)
        {
            ...
            if (isValidTs())
            {
                ...
                if (timeinMinutes >= 0)
                {
                    ...
                    {
                        MySqlCommand cmd = new MySqlCommand("INSERT INTO TimeSlot VALUES('',@Day, @Timeslot, @Occupied)", con);
                        cmd.CommandType = CommandType.Text;
